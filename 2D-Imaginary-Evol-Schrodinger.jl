...
potentialArray = spzeros(Int64(i), Int64(i)) #Potential values at each point on the grid
...
function IC(xArray, yArray)
    ...
    for i in eachindex(xArray)
        zArray[i,:] = 2 .*sin(π.*(xArray[i])) .* sin.(π.*yArray) #Infinite Square Well Wavefunction
